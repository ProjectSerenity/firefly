...
func findWords(chars []pdf.Text) (words []pdf.Text) {
	...
	for i := 0; i < len(chars); {
		...
		for k := i; k < j; {
			...
			f = strings.TrimSuffix(f, "-Italic")
			words = append(words, pdf.Text{Font: f, FontSize: ck.FontSize, X: ck.X, Y: ck.Y, W: end, S: s})
			k = l
		}
		i = j
	}
...
func findEncodingTable(text []pdf.Text) [][]string {
	...
	for i, t := range text {
		...
		if match(t, "NeoSansIntel", 9, "Op/En") || match(t, "NeoSansIntel", 9, "Operand") ||
			match(t, "NeoSansIntelMedium", 9, "Op/En") || match(t, "NeoSansIntelMedium", 9, "Operand") ||
	...
	for _, t := range text {
		...
		t.S = strings.Replace(t.S, "ModRM:rm", "ModRM:r/m", 1)
		line[i] += t.S
	}
...
func processListing(p *listing, insts *[]*instruction) {
	...
	encs := make(map[string]OperandEncoding)
	for _, table := range p.enctables {
		for _, row := range table[1:] {
			tuple := ""
			firstOperand := 1
			if len(table[0]) >= 2 && strings.Contains(table[0][1], "Tuple") {
				tuple = row[1]
				firstOperand = 2
			}

			for len(row[firstOperand:]) > 0 && (row[len(row)-1] == "NA" || row[len(row)-1] == "N/A" || row[len(row)-1] == "" || row[len(row)-1] == " source") {
				row = row[:len(row)-1]
			}

			encs[row[0]] = OperandEncoding{Tuple: tuple, Operands: row[firstOperand:]}
		}
	}
	...
	for _, table := range p.mtables {
		...
		for _, row := range table[1:] {
			...
			if len(row) >= 5 && row[1] == "CMOVG r64, r/m64" && row[3] == "V/N.E." && (row[4] == "NA" || row[4] == "N/A") {
				row[3] = "V"
				row[4] = "N.E."
			}
			...
			for i, hdr := range heading {
				...
				case "Op/En":
					...
					if inst.args == nil && inst.syntax == "PREFETCHW m8" && x == "A" && len(encs) == 1 && encs["M"].Operands != nil {
						opEn = encs["M"]
					}

					inst.tuple = opEn.Tuple
					inst.args = opEn.Operands
