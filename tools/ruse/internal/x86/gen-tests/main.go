...
func genTests(workers int) error {
	...
	name := filepath.Join("tools", "ruse", "compiler", "testdata", "x86-tests.csv")
	...
	w := csv.NewWriter(f)
	err = w.Write([]string{"uid", "mode", "code", "ruse", "intel"})
	if err != nil {
		return fmt.Errorf("failed to write header to %q: %v", name, err)
	}

	for _, entry := range entries {
		err = w.Write([]string{entry.Inst.UID, entry.Mode.String, entry.Code, entry.Ruse, entry.Intel})
		if err != nil {
