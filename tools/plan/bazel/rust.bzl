...
def _plan_rust_shared_impl(ctx):
    out = ctx.actions.declare_file(ctx.label.name + ".rs")
    cmd = "{plan} build -arch {arch} -rustfmt {rustfmt} -rust-shared {dst} {src}".format(
        plan = ctx.executable._plan.path,
        arch = ctx.attr.embed[Plan].arch,
        rustfmt = ctx.executable.rustfmt.path,
        src = ctx.attr.embed[Plan].path.path,
...
plan_rust_shared = rule(
    _plan_rust_shared_impl,
    attrs = {
        "embed": attr.label(
            doc = "A Plan document.",
            mandatory = True,
    ...
    doc = "Translates a Plan document to the equivalent Rust code for shared data structures.",
)
